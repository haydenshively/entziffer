...
export function readConfig(path: string, quiet = false): Config | null {
  let raw: string;
  try {
  ...
  return validate(parsed, path, quiet);
}

export function requireConfig(path: string, quiet = false): Config {
  const cfg = readConfig(path, quiet);
  if (cfg === null) {
    throw new CliError(
      "E_CONFIG_MISSING",
...
function validate(parsed: unknown, path: string, quiet: boolean): Config {
  ...
  if (o.default !== undefined && typeof o.default !== "string") {
    throw configError(path, "'default' must be a string");
  }
  for (const key of Object.keys(o)) {
    if (!KNOWN_KEYS.has(key)) warn(`ignoring unknown config key '${key}' in ${path}`, quiet);
  }
  ...
  if (typeof o.default === "string") cfg.default = o.default;
  return cfg;
}
...
export function warnIfPermissive(path: string, quiet: boolean): void {
  try {
    const mode = statSync(path).mode & 0o777;
    if ((mode & 0o077) !== 0) {
      warn(`config ${path} is mode ${mode.toString(8)}; run: chmod 600 ${path}`, quiet);
    }
  } catch {
